...
let navBarCoordsTop = navBar.getBoundingClientRect().top + document.documentElement.scrollTop;
...
window.addEventListener('resize', () => {
    if (!navBar.classList.contains('navbar-stick')) {
        navBarCoordsTop = navBar.getBoundingClientRect().top + document.documentElement.scrollTop;
    } else {
        navBarCoordsTop = myName.getBoundingClientRect().bottom + document.documentElement.scrollTop + 13;
    }
    myNameCoordsTop = myName.getBoundingClientRect().top + document.documentElement.scrollTop;
});
...
document.addEventListener('scroll', () => {
    ...
    } else if (document.documentElement.scrollTop === 0 && arrow.style.opacity != '1') {
...
const listOfClassesToTheme = ['about-bracket', 'about-tag', 'about-attribute', 'about-attribute-value', 'about-section', 'about-backdrop', 'about-text', 'project-link', 'project-card', 'project-card-img-container', 'contact-section'];
...
function toggleDarkThemeOnClasses(addDark) {
    if (addDark) {
        for (let i = 0; i < listOfClassesToTheme.length; i++) {
            document.querySelectorAll(`.${listOfClassesToTheme[i]}`).forEach((elem) => {
